...
export default function BattleScreen() {
  ...
  const nearbyEnemies = enemies.slice(0, 5);

  const handleEngage = (enemy: typeof enemies[0]) => {
    encounterEnemy(enemy);
    setBattleLog(prev => [`⚔️ 遭遇 ${enemy.name}！`, ...prev].slice(0, 8));
  };
  ...
  const handleAttack = () => {
    ...
    if (playerHit > 0) {
      lines.push(`👹 ${currentEnemy.name} 反擊！受到 ${playerHit} 傷害`);
    }

    if (currentEnemy.currentHp - (result.hit ? result.dmg : 0) <= 0) {
      lines.push(`🏆 擊敗 ${currentEnemy.name}！`);
      SFX.enemyDie(); // victory sound
    }

    setBattleLog(prev => [...lines, ...prev].slice(0, 8));
  };
          ...
          <View style={styles.battleZone}>
            <View style={styles.enemyCard}>
              ...
              <View style={styles.enemyHpBar}>
                <View style={[styles.enemyHpFill, { width: `${(currentEnemy.currentHp / currentEnemy.hp) * 100}%` }]} />
              </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => useGameStore.getState().skipEnemy()}
            >
